...
import { validateRequest, generateBooks } from './utils/helpers';
...
app.get(booksRoute, (req: Request, res: Response) => {
    ...
    try {
        const books = generateBooks(seed as string, Number(page), lang as string, Number(likes), Number(reviews));
        return res.json(books);
    } catch {
        return res.status(StatusCodes.internalServerError).json({ error: Errors.generatingBooks });
    }
});
